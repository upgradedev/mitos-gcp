...
class NotAuthorisedToSetUp(Exception):
    """This caller may not configure the GitHub App for this deployment."""


def _require_setup_authority(request: Request) -> None:
    """Who is allowed to bind a GitHub App to this deployment.

    Nobody was. `/github/app/new` answered 200 to anyone, and the callback
    checked only `mitos_github_manifest_state` against a cookie the same
    response had just set. That is a CSRF check, and it was doing duty as an
    authorisation check.

    The consequence is a complete takeover by an anonymous visitor: load the
    page, click through, create an App under YOUR OWN GitHub account, and
    GitHub redirects back with a code and the state you were given. The
    callback then writes your private key, your client secret and your webhook
    secret into this deployment's Secret Manager and overwrites the Firestore
    record. After that you hold the webhook secret, so the reader accepts your
    signed deliveries, and the reader mints installation tokens with your key.

    A shared secret rather than a session, deliberately. Before any App exists
    there are no workspaces and no owners, so an owner check has nothing to
    check against, and "the first person to arrive becomes the owner" is the
    same hole with a longer sentence. The token is generated by Terraform and
    lives only in this service's environment.

    Absent configuration refuses rather than allows. A deployment that has not
    been given a token cannot be set up, which is the safe direction: the
    alternative is that forgetting to configure it reopens the hole silently.
    """
    expected = (os.environ.get("MITOS_SETUP_TOKEN") or "").strip()
    if not expected:
        raise NotAuthorisedToSetUp(
            "this deployment has no MITOS_SETUP_TOKEN, so nobody can configure "
            "its GitHub App. Terraform generates one; read it with "
            "`terraform -chdir=infra output -raw setup_token`."
        )
    offered = (
        request.headers.get("x-mitos-setup-token")
        or request.query_params.get("setup_token")
        or ""
    ).strip()
    if not offered or not secrets.compare_digest(offered, expected):
        raise NotAuthorisedToSetUp(
            "a setup token is required to bind a GitHub App to this deployment"
        )


@app.exception_handler(NotAuthorisedToSetUp)
async def _not_authorised_to_set_up(request, exc: NotAuthorisedToSetUp):
    return JSONResponse(
        ...
        content={
            ...
            "detail": str(exc),
            "why": (
                "the manifest callback writes a private key, a client secret and "
                "a webhook secret into this deployment. Whoever completes it "
                "controls what this fleet trusts."
            ),
        },
    )
...
def github_app_new(request: Request) -> HTMLResponse:
    ...
    # Before anything is rendered. The state cookie set below is what the
    # callback trusts, so the authority has to be proven here, where it can be
    # asked for: GitHub's redirect back cannot carry a header of ours.
    _require_setup_authority(request)
    state = secrets.token_urlsafe(32)
...
def thread(limit: int = 100) -> dict[str, Any]:
    """The demo corpus thread, and only ever that.

    This returned `ledger().all()` unfiltered, and the webhook handler appends
    the analysis of every real pull request to that same ledger. Nothing leaked
    while no real analysis had run, which is exactly why it was invisible: the
    endpoint would have started publishing repository names, findings and
    provenance for private repositories the moment the product was used for the
    first time, anonymously, to anyone.

    So it filters. An entry that names a repository belongs to somebody's
    workspace and is served by `/api/workspace/thread`, which requires a session
    and scopes by membership. What is left here is the synthetic corpus the
    recorded demo and every offline test use, which is what this endpoint is
    for.

    Filtered rather than closed, because a judge with no account should still be
    able to follow a thread end to end, and that was the reason it was opened in
    the first place.
    """
    _require_demo_mode()
    ...
    return {
        ...
        "scope": (
            "the built-in demo corpus. Entries belonging to a workspace are "
            "served by /api/workspace/thread, which requires a session."
        ),
    }
