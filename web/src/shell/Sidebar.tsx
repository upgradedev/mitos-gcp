  ...
  return (
    <aside className={`${open ? "w-64" : "w-0"} sticky top-0 h-screen shrink-0 overflow-hidden border-r border-ink-800 bg-[#0b0d12] transition-[width]`}>
      {!open ? null : <div className="flex h-full w-64 flex-col">
        ...
        <div className="mt-auto border-t border-ink-800 p-3"><a href="#/settings" onClick={(event) => { event.preventDefault(); onNavigate("settings"); }} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${route === "settings" ? "bg-ink-800 text-ink-50" : "text-ink-400 hover:bg-ink-900"}`}><Settings size={17} /> Settings</a><div className="mt-3 flex items-center gap-3 rounded-lg px-3 py-2"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-ink-800 text-xs font-semibold text-cyan-300">MT</div><div className="min-w-0"><p className="truncate text-xs font-medium text-ink-200">Mitos team</p><p className="text-[11px] text-ink-600">Workspace</p></div></div></div>
      </div>}
    </aside>
  );
}
