...
| a second model family reviews the draft independently | **proven in CI, both directions.** Gemma 4 26B A4B IT, served by Google Cloud managed open models, reviews a sanitised draft and can only add advisories a human reads. It cannot approve, cannot remove a deterministic finding and cannot clear the injection flag — a property of `_with_critic`, not of the prompt, asserted in `tests/unit/test_the_second_model_cannot_approve.py` and checked by mutating the invariant seven ways. The managed endpoint answered this project on 2026-08-30: requested and returned `google/gemma-4-26b-a4b-it-maas`, 200 in 0.51s. **live**, `MITOS_CRITIC_MODEL=google/gemma-4-26b-a4b-it-maas`, exercised in CI on every run of this branch by `tests/integration/test_gemma_live.py`, with a step that fails if that suite silently skips. The live call is not only a 200: it asserts the model that answered is the model that was asked for, that a real model reading a real prompt injection still cannot change the verdict, and that the digest of what was actually sent equals the digest of the sanitiser's output for the same input — so the raw draft demonstrably did not leave the process. Measured against this project: 5.3s, 344 tokens, two genuine advisories raised on a draft with no migration and no rollback plan, verdict and findings unchanged |
